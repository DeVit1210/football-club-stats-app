...
void Game::show_match_squad(vector<IdentityData>& vec_of_players)
{
	...
	{
		int j = find_same_id(m_squad_players_id[i], vec_of_players);
		cout << "|" << setw(9) << vec_of_players.at(j).get_id()
			<< "|" << setw(14) << vec_of_players.at(j).get_surname()
			<< "|" << setw(14) << vec_of_players.at(j).get_name() << endl;
...
void Game::set_scored_quantity()
{
	...
	while (!checker)
	{
		...
		if (cin.fail())
		{
			...
			cout << "\n  К сожалению, нельзя забить отрицательное количество голов!" << TRY_IT_AGAIN;
...
{
	cout << "\n  Введите количество пропущенных командой голов" << endl;
	bool checker(false);
	while (!checker)
	{
...
void Game::add_cards_in_stats(vector<Statistics>& vec_of_players_stats, vector<IdentityData>& vec_of_players_data)
{
	...
	while (cards_amount != m_red_cards_quantity)
	{
		...
		if (player_num && is_already_in_the_squad(player_num, PLAYERS_AMOUNT))
		{
			...
			else cout << "\n  Игрока с таким номером в составе на матч нет!" << TRY_IT_AGAIN << endl;
		}
		else cout << NO_PLAYER_IN_THE_SQUAD << endl;
		push_and_clear_page();
	}
