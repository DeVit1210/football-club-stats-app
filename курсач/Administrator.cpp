		...
		else
		{
			for (int i = 0; i < vec_of_players_data.size(); i++)
			{
				if (vec_of_players_data.at(i).get_id() == id)
				{
					cin.clear();
					cin.ignore(32767, '\n');
					cout << "Игрок с таким номером уже есть! Введите другой id" << endl;
					continue;
				}
			}
		}
...
{
	cout << "====================================================================" << endl;
	cout << "| Дата игры |  Соперник  |   Cчет   | Итог игры | Желтые | Красные |" << endl;
	cout << "====================================================================" << endl;
...
{
	cout << "Введите номер игрока, данные о котором необходимо удалить" << endl;
	...
	{
		cout << "К сожалению, игрока с таким номером в команде нет!" << endl;
		push_and_clear_page();
		return;
	}
