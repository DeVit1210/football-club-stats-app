...
const string WARNING = " \n  Изменение данных в этом поле просходит через добавление результатов игры!\n  Вы точно хотите продолжить?(y/n): ";
...
public:
	...
	friend void write_stats_data(vector<Statistics>& vec_of_players);
	...
	friend istream& operator>>(istream& fstats, Statistics& obj);
	friend ostream& operator<<(ostream& fstats, const Statistics& obj);
};
...
public:
	...
	friend void write_players_data(vector<IdentityData>& vec_of_players);
	...
	void change_date();
	friend istream& operator>>(istream& fplayers, IdentityData& obj);
	friend ostream& operator<<(ostream& fplayers, const IdentityData& obj);
};
